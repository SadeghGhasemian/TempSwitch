...
# Configuration
# ============================================
base_dir = r'E:\TemporalSwitch\HDDM_latest\power_analysis_bis_z' # for full model and different sample sizes
#base_dir = r'E:\TemporalSwitch\HDDM_latest\power_analysis_bis_z\full_N35'
data_path = os.path.join(base_dir, 'bisDDM.csv')
models_base = os.path.join(base_dir, 'downsampling_models') # for full model and different sample sizes
...
# Preprocessing: multiply by 10 and center
data["duration"] = data["duration"] * 10
mean_duration = data["duration"].mean()
data["duration"] = data["duration"] - mean_duration

# ============================================
# Regression model – now includes all parameters
# ============================================
reg_model = [
    "a ~ 1",                                           # boundary separation (intercept only)
    "v ~ 1",                                           # drift rate (intercept only)
    "t ~ 1",                                           # non‑decision time (intercept only)
    "z ~ duration + C(hist, Treatment('R')) + C(hist, Treatment('R')):duration"
]
...
# ============================================
sample_sizes = [10, 15, 20, 25, 30, 35] # for full model and different sample sizes
#sample_sizes = [35]
n_chains = 4
