...
if __name__ == '__main__':
    # ------------------------------------------------------------------------
    # PATHS – adjust to your actual output folder (e.g., with 28ms unit)
    # ------------------------------------------------------------------------
    base_dir = r"E:\TemporalSwitch\Pymc_reaction_time"
    output_dir = os.path.join(base_dir, "output_rt_toj_centered_28ms")   # <-- updated folder name
    ...
    print("=" * 70)
    print("POSTERIOR PREDICTIVE CHECK – REACTION TIME (TOJ, 28ms UNIT, MEAN‑CENTERED)")
    print("=" * 70)
    ...
    with open(center_json, 'r') as f:
        center = json.load(f)
    soa_mean = center['soa_mean']       # mean of SOA in 28 ms units
    ...
    # Convert to 28 ms units, then center
    soa_28 = soa_original / 28.0
    soa_c = soa_28 - soa_mean
    ...
    for cond in [0, 1]:
        ...
        pred_rt = np.zeros((len(soa_c), n_draws))
        for i, sc in enumerate(soa_c):
