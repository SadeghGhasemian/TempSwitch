...
# Configuration
# ============================================
base_dir = r'E:\TemporalSwitch\HDDM_latest\power_analysis_bis_z'
...
params_of_interest = [
    'z_Intercept',
    "z_C(hist, Treatment('R'))[T.S]",
    'z_duration',
    "z_C(hist, Treatment('R'))[T.S]:duration"
]
...
# ============================================
# Violin plots across sample sizes
# ============================================
...
for param in params_of_interest:
    ...
    for n in sample_sizes:
        if n in combined_traces and param in combined_traces[n].columns:
            vals = combined_traces[n][param].dropna().values
    ...
    if not data_for_violin:
        ...
        plt.close()
        continue
    parts = plt.violinplot(data_for_violin, positions=positions, showmeans=True, showmedians=False)
    ...
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(results_base, f'violin_{param}.png'), dpi=150)
    plt.close()
